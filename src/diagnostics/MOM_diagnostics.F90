...
subroutine post_surface_dyn_diags(IDs, G, diag, sfc_state, ssh)
  ...
  if ((IDs%id_ssu_deconv > 0) .and. allocated(sfc_state%u_deconv)) &
...
subroutine post_surface_thermo_diags(IDs, G, GV, US, diag, dt_int, sfc_state, tv, &
  ...
  if (tv%T_is_conT) then
    ...
    if ((IDs%id_sstcon_deconv > 0) .and. allocated(sfc_state%SST_deconv)) &
      call post_data(IDs%id_sstcon_deconv, sfc_state%SST_deconv, diag)
    ...
    if (IDs%id_sst_deconv > 0) then
      if (allocated(sfc_state%SSS_deconv) .and. allocated(sfc_state%SST_deconv)) then
        ...
        enddo
        call post_data(IDs%id_sst_deconv, work_2d, diag)
      endif
    endif
  else
    ! Internal T&S variables are potential temperature & practical salinity
    ...
    if ((IDs%id_sst_deconv > 0)  .and. allocated(sfc_state%SST_deconv)) &
      call post_data(IDs%id_sst_deconv, sfc_state%SST_deconv, diag)
  endif
  ...
  if (tv%S_is_absS) then
    ...
    if ((IDs%id_sssabs_deconv > 0)  .and. allocated(sfc_state%SSS_deconv)) &
      call post_data(IDs%id_sssabs_deconv, sfc_state%SSS_deconv, diag)
